...
def ResidualBlock(inputs,n_outputs,k,d,dropout_rate):
    ...
    f = Add()([e,d2])
    g = Dense(n_outputs, activation='relu',name='ResidBlock_'+str(d),\
              kernel_initializer=RandomNormal(mean=0,stddev=0.01))(f)
    return g


def TCN(input_dim,time_steps,layers,features,kernel_size=3, dropout=0.):
    ...
    mod1 = Flatten()(cEnd)
    mod2 = Dense(1,activation='linear',kernel_initializer=RandomNormal(mean=0,stddev=0.01))(mod1) # the last output should be able to reach all of y values
    ...
    while True:
        ...
        for i in range(num_batch):
            ...
            for j,(EQ,day) in enumerate(zip(number_EQ[start:start+batch_size],number_days[start:start+batch_size])):
                data += [np.load(PATH+'/EQ'+str(EQ)+'_'+str(num_days-day-1)+'daysuntilEQ.npy')]
                y    += [num_days-day-1]
